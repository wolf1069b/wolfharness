...
``wiki_external_expert`` role) now live in :mod:`wolfharness.capabilities.viking.ticket`;
...
def build_tools(cap: WikiBuildCapability) -> list[Callable[..., Any]]:
    ...
    - ``wiki_external_expert`` → the ticket tools from
      :mod:`wolfharness.capabilities.viking.ticket`
      (``build_ticket_tools``).
...
def get_instructions(role: str | None) -> str | None:
    ...
    from :mod:`wolfharness.capabilities.viking.ticket`.  For all other
    roles, returns ``None`` (instructions come from the agent's system
    prompt).
    """
    if role == "wiki_external_expert":
        from wolfharness.capabilities.viking.ticket import get_ticket_instructions
