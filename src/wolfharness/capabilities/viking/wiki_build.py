...
def _build_tool_fns(
    ...
    *,
    ...
    for name in names:
        ...
        # ctx keeps its RunContext annotation so pydantic-ai strips it from
        # the model schema; every other parameter gets a resolvable ``Any``
        # (the host methods are ``from __future__ import annotations`` and
        # their lazy string types cannot resolve in this module's globals).
        wrapper.__annotations__ = (
            {"ctx": RunContext[Any]}
            | {p: Any for p in new_sig.parameters if p != "ctx"}
            | {"return": Any}
        )
...
class WikiBuildCapability(AbstractCapability[Any]):
    ...
    def _ensure_tools(self) -> None:
        ...
        from xeno_adp_agentic.wiki.serve.mcp_server import (
            WikiBuildTools,
        )
    ...
    def get_toolset(self) -> AgentToolset[Any] | None:
        ...
        if not self._tool_fns:
            return None
        toolset: AgentToolset[Any] = FunctionToolset(self._tool_fns, id="wiki_build")
