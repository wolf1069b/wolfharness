"""OPAMixin — OPA conflict/problem record handling for WikiBuildTools.

Extracted from ``mcp_server.py``.  OPA records live under ``OP/OpA/``,
...
_RELATION_GAP_CODES: frozenset[str] = frozenset({
    ...
    # Relation-type errors caused by build bugs (broken relation sync),
    # not genuine knowledge conflicts — suppress from fact_conflict OPA flooding.
    "typed_relation_wrong_concept",
})
...
_SECTION_ALIASES: dict[str, tuple[str, ...]] = {
    "affected_components": (SECTION_IMPACT_SCOPE,),
    "verification_procedures": (SECTION_VERIFICATION,),
    "repair_procedures": (SECTION_REPAIR_METHOD,),
    "related_faults": (SECTION_POSSIBLE_FAILURE,),
    "possible_faults": (SECTION_POSSIBLE_FAILURE,),
    "assembly_parts": (SECTION_OVERVIEW,),
...
class OPAMixin(WikiBuildDeps):
    """OPA conflict/problem record tools mixin for ``WikiBuildTools``."""
    ...
    def _find_op_key(self, concept: str, record_id: str) -> str | None:
        ...
        if self.store.read_text(flat) is not None:
            return flat
        for key in self.store.list_dir(base, recursive=True):
            if key.endswith(f"/{record_id}.md"):
                if self.store.read_text(key) is not None:
                    return key
        return None
    ...
    def _record_from_content(content: str) -> dict[str, object]:
        """Parse OP record: frontmatter + H1 title + body sections merged.

        Prose fields may live in YAML (legacy) or body (new format); body is
        the fallback so both old and new files parse identically. Machine
        fields always win from frontmatter.
        """
        record: dict[str, object] = dict(parse_frontmatter(content or ""))
        ...
        for field, section in (
            ...
            if not record.get(field) and section in sections:
                record[field] = sections[section]
        return record
    ...
    def _readable_slug(value: str, *, fallback: str) -> str:
        ...
        return OPAMixin._clip_utf8(slug, 60).rstrip("-._") or fallback
    ...
    def _readable_opa_id(
        ...
        *,
        ...
        The issue type, the target name, and the title slug go into the
        filename so the problem is recognizable at a glance; the dedupe
        hash keeps it stable per logical issue.
        """
        ...
        title_slug = self._readable_slug(title, fallback="issue")[:20].rstrip("-._") or "issue"
        base = dedupe_key.removeprefix("opa-dedupe-") if dedupe_key else ""
        suffix = base[:8] if len(base) >= 6 else ""
        if not suffix:
    ...
    def create_opa(
        ...
        *,
        ...
        if effective_reason_code not in OPA_REASON_CODES:
            raise ValueError(
                f"Unsupported OPA reason_code: {effective_reason_code!r}; expected one of {', '.join(OPA_REASON_CODES)}",
            )
    ...
    def create_ops(
        ...
        *,
        ...
        if missing_hits:
            raise ValueError(
                f"OPS retrieved_uris were not returned by the server-side retrieval receipt: {missing_hits}",
            )
    ...
    def _ops_record(self, value: str) -> tuple[str, str, dict[str, object]]:
        ...
        for path, content in self._op_files("OPS"):
            if Path(path).stem == ops_id:
                return ops_id, path, self._record_from_content(content)
        ...
        *,
        ...
        if source_type != "external_expert" and not archive_reason.strip():
            raise ValueError(
                ...
                "create OPL records. Provide archive_reason to archive an explicit manual snapshot.",
            )
        unconfirmed_ops = [
            uri
            for uri in ops_uris
    ...
    def apply_opl(self, opl_id: str) -> dict[str, object]:
        ...
        if current is not None:
            ...
            return self._update_external_opl_status(
                ...
                status="unconfirmed",
                apply_status="failed",
                apply_error="candidate_operations cannot create a missing target without candidate_content.",
            )
        try:
            entity_uri = self.merge_entity(
                concept,
    ...
    def op_flow_status(self, *, limit: int = 500) -> dict[str, object]:
        ...
        # file once per pending OPA, turning a quality gate into OPA×OPS remote
    ...
    def refine_opa_reason_code(
        ...
        *,
        ...
        if reason_code not in OPA_REASON_CODES:
            raise ValueError(
                f"Unsupported reason_code: {reason_code!r}; expected one of {', '.join(OPA_REASON_CODES)}",
            )
        key = self._find_opa_key(opa_id)
        if key is None:
            raise FileNotFoundError(f"OPA not found: {opa_id}")
